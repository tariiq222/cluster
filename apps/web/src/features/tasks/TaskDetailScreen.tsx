...
export function TaskDetailScreen({ taskId }: { taskId: string }) {
  ...
  const { update, transition, addComment } = useTaskMutations()
  ...
  const busy = update.isPending || transition.isPending || addComment.isPending || attachBusy
  ...
  }, [commentsQuery, taskQuery])

  const submitTransition = useCallback(
    async (action: TransitionAction) => {
      if (!task) return
      let input: generated.TaskTransitionRequest | undefined
    ...
    [handleStale, noteDraft, reasonDraft, t.actionError, t.noteRequired, t.reasonRequired, task, transition],
  )
  ...
  }, [editDescription, editPriority, editTitle, handleStale, t.actionError, t.descriptionTooLong, t.titleRequired, t.titleTooLong, task, update])
  ...
  const submitComment = useCallback(async () => {
    ...
    setActionError(null)
    try {
      await addComment.mutateAsync({ taskId: task.id, input })
      void queryClient.invalidateQueries({ queryKey: ['task-comments', taskId] })
      setCommentDraft('')
      setMentionsDraft('')
      setDialog(null)
  ...
  }, [addComment, commentDraft, mentionsDraft, queryClient, t.actionError, t.commentRequired, task, taskId])
  ...
  }, [csrfToken, documentDraft, queryClient, t.actionError, t.documentIdRequired, task, taskId])
  ...
  if ((screenState === 'error' || screenState === 'conflict' || screenState === 'stale') && !task) {
    return (
      <Page aria-labelledby="task-detail-heading">
        <PageHeader id="task-detail-heading" title={t.pageTitle} actions={backAction} />
