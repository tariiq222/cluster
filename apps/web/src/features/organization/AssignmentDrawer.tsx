  ...
  return (
    <Drawer open={open} onClose={onClose} title={text.createAssignment} ariaLabelClose={text.close} dismissable={!submitting}>
      <form className="org-drawer-form" onSubmit={(event) => void submit(event)} noValidate>
        ...
        <Field id="assignment-start" label={text.startAt} required error={error === 'validation' && !startAt ? text.validation : undefined}><input id="assignment-start" aria-label={text.startAt} type="datetime-local" value={startAt} required aria-required="true" aria-invalid={error === 'validation' && !startAt || undefined} onChange={(event) => setStartAt(event.target.value)} /></Field>
        <Field id="assignment-end" label={text.endAt}><input id="assignment-end" aria-label={text.endAt} type="datetime-local" value={endAt} onChange={(event) => setEndAt(event.target.value)} /></Field>
