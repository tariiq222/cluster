...
export function CalendarsSection() {
  ...
  const fetcher = useCallback(() => listBusinessCalendars(csrfToken), [csrfToken])
  const { state, data, reload } = useSectionLoad(fetcher, (payload: BusinessCalendarList) => isEmptyCollection(payload.items), scopeEpoch)
  ...
  const save = useCallback(async () => {
    ...
    try {
      if (edit.kind === 'create') {
        await createBusinessCalendar(csrfToken, {
        ...
        if (edit.kind === 'weekday') {
          await setBusinessCalendarWeekday(
            csrfToken,
            calendar.id,
            edit.weekday,
          ...
          await setBusinessCalendarException(
            csrfToken,
            calendar.id,
            exceptionDate,
            {
  ...
  }, [csrfToken, edit, fail, isWorkingDay, startsAt, endsAt, scopeId, scopeType, exceptionDate, exceptionEndsOn, exceptionType, reason, locale, reload])

  const publish = useCallback(async (calendar: BusinessCalendarEntity) => {
    setActionBusy(true)
    setActionError(null)
    setActionNotice(null)
    try {
      await publishBusinessCalendar(csrfToken, calendar.id, calendar.lock_version)
