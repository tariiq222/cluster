...
export function LogsSection() {
  ...
  const fetcher = useCallback(
    () => listPlatformTechnicalLogs(csrfToken, { per_page: 50 }),
    [csrfToken],
  )
  const { state, data, reload } = useSectionLoad(fetcher, (payload: PlatformTechnicalLogList) => isEmptyCollection(payload.items), scopeEpoch)
  ...
  const submitRestore = useCallback(async () => {
    ...
    try {
      await requestPlatformTechnicalLogsRestore(csrfToken, { manifest_id: manifestId.trim(), reason: reason.trim() })
  ...
  }, [csrfToken, fail, locale, manifestId, reason])
    ...
    <>
      ...
      <Drawer open={restoreOpen} onClose={() => setRestoreOpen(false)} title={t(logsCopy.restore, locale)}>
        ...
        <div className="form-actions">
          ...
          <Button
            variant="primary"
            disabled={actionBusy || manifestId.trim() === '' || reason.trim() === ''}
