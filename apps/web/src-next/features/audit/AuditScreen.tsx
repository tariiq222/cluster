...
import type { AuditEvent, AuditExportDescriptor, AuditIntegrityResult } from '../../../src/api/generated/cluster'
...
export function AuditScreen() {
      ...
      try {
        const page = unwrap<generated.AuditEventCollection>(
          await generated.listAuditEvents(
            { ...queryFromFilters(filters), limit: 50, ...(cursor ? { cursor } : {}) },
        ...
        if (!append) setItems([])
        setLoadError(apiMessage(error, t.loadFailed))
      } finally {
        setLoading(false)
        setLoadingMore(false)
      }
    },
    [csrfToken, filters, t.loadFailed],
  )
  ...
  async function submitExport(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault()
    setExportBusy(true)
    ...
    try {
      ...
      setCreatedExport(
        unwrap<AuditExportDescriptor>(
          await generated.createAuditExport(
            {
              format: exportFormat,
  ...
  async function submitVerification(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault()
    ...
    try {
      setVerification(
        unwrap<AuditIntegrityResult>(
          await generated.verifyAuditIntegrity(
            {
  ...
  return (
    <Page aria-labelledby="audit-title">
      ...
      <Panel id="audit-ledger" title={t.ledger} level={2}>
        ...
        {!loading && loadError ? (
          <InlineError message={loadError} retryLabel={t.retry} onRetry={() => void loadPage(null, false)} />
        ) : null}
        ...
        {nextCursor ? (
          <div className="pagination-bar">
            <Button variant="secondary" disabled={loadingMore} onClick={() => void loadPage(nextCursor, true)}>
              {t.loadMore}
            </Button>
          </div>
        ) : null}
      ...
      <div className="panel-grid">
        {canExport ? (
          <Panel id="audit-export" title={t.exportTitle} level={2}>
            <form className="inline-form" onSubmit={(event) => void submitExport(event)}>
        ...
        {canVerify ? (
          <Panel id="audit-integrity" title={t.verifyTitle} level={2}>
            <form className="inline-form" onSubmit={(event) => void submitVerification(event)}>
