...
export function DocumentsScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const collection = unwrap<generated.EntityCollection>(
        await generated.listDocuments(params, requestInit(csrfToken)),
      )
      if (request !== requestRef.current) return
      const page = collection.items as unknown as DocumentSummary[]
      setItems(page)
  ...
  }, [classification, csrfToken, loadingMore, nextCursor])
