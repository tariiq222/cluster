...
function useLoadState() {
  ...
  const begin = useCallback(() => {
    ...
    return {
      epoch,
      alive: () => activeRef.current && epoch === requestRef.current,
    }
  }, [])
  ...
  return { loading, state, begin, finish, fail }
}
...
function OverviewTab() {
  ...
  const { loading, state, begin, finish, fail } = useLoadState()
  const [cluster, setCluster] = useState<generated.Cluster | null>(null)
  const [facilities, setFacilities] = useState<generated.Facility[]>([])
  ...
  const load = useCallback(async () => {
    const { alive } = begin()
    try {
      let clusterValue: generated.Cluster | null = null
      try {
        clusterValue = unwrap<generated.Cluster>(
          await generated.getCluster(requestInit(token)),
        )
      } catch (caught) {
        if (!(caught instanceof ApiError && caught.status === 404)) throw caught
      }
      const facilityPage = unwrap<generated.FacilityCollection>(
        await generated.listFacilities({ limit: 100 }, requestInit(token)),
      )
      if (!alive()) return
        ...
        <FacilityDrawer
          ...
          onSaved={(saved) => {
            setFacilities((current) =>
              current.some((facility) => facility.id === saved.id)
                ? current.map((facility) =>
                    facility.id === saved.id ? saved : facility,
                  )
                : [...current, saved],
            )
            setDrawer({ kind: 'closed' })
...
function ClusterDrawer({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFailure(null)
    try {
      let saved: generated.Cluster
      if (editing && cluster) {
        const fresh = unwrap<generated.Cluster>(
          await generated.getCluster(requestInit(token)),
        )
        saved = unwrap<generated.Cluster>(
        ...
        saved = unwrap<generated.Cluster>(
          await generated.createCluster(
            { code, name: name.trim(), name_en: nameEn.trim() || null },
...
function StructureTab() {
  ...
  const [cluster, setCluster] = useState<generated.Cluster | null>(null)
  const [facilities, setFacilities] = useState<generated.Facility[]>([])
  const [units, setUnits] = useState<generated.OrganizationUnit[]>([])
  const [positions, setPositions] = useState<generated.Position[]>([])
  const [jobTitles, setJobTitles] = useState<generated.JobTitle[]>([])
  const [notice, setNotice] = useState<string | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [clusterValue, facilityPage, unitPage, positionPage, jobTitlePage] =
        await Promise.all([
          generated.getCluster(requestInit(token)),
          generated.listFacilities({ limit: 100 }, requestInit(token)),
          generated.listOrganizationUnits({ limit: 100 }, requestInit(token)),
          generated.listPositions({ limit: 100 }, requestInit(token)),
          generated.listJobTitles({ limit: 100 }, requestInit(token)),
        ])
      if (!alive()) return
      setCluster(unwrap<generated.Cluster>(clusterValue))
      setFacilities(unwrap<generated.FacilityCollection>(facilityPage).items)
      setUnits(unwrap<generated.OrganizationUnitCollection>(unitPage).items)
      setPositions(unwrap<generated.PositionCollection>(positionPage).items)
      setJobTitles(unwrap<generated.JobTitleCollection>(jobTitlePage).items)
      finish()
    } catch (caught) {
      if (!alive()) return
      setCluster(null)
  ...
  async function handleReorder() {
    ...
    setNotice(null)
    try {
      const result = unwrap<{ updated: number; policy: string }>(
    ...
    <>
      ...
      {cluster ? (
        <UnitDrawer
          ...
          units={units}
          onSaved={(saved) => {
            setUnits((current) => [...current, saved])
      ...
      <PositionDrawer
        ...
        onSaved={(saved) => {
          setPositions((current) => [...current, saved])
          setPositionDrawerOpen(false)
          setPreselectedUnitId(undefined)
...
function UnitDrawer({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFailure(null)
    try {
      const created = unwrap<generated.OrganizationUnit>(
...
function PositionDrawer({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFailure(null)
    try {
      const created = unwrap<generated.Position>(
...
function PeopleTab() {
  ...
  const capabilities = useCapabilities()
  const { loading, state, begin, finish, fail } = useLoadState()
  const [people, setPeople] = useState<generated.Person[]>([])
  const [positions, setPositions] = useState<generated.Position[]>([])
  const [assignments, setAssignments] = useState<generated.Assignment[]>([])
  const [notice, setNotice] = useState<string | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [peoplePage, positionPage, assignmentPage] = await Promise.all([
        generated.listPeople({ limit: 100 }, requestInit(token)),
        generated.listPositions({ limit: 100 }, requestInit(token)),
        generated.listAssignments({ limit: 100 }, requestInit(token)),
      ])
      if (!alive()) return
      setPeople(unwrap<generated.PersonCollection>(peoplePage).items)
      setPositions(unwrap<generated.PositionCollection>(positionPage).items)
      setAssignments(
        unwrap<generated.AssignmentCollection>(assignmentPage).items,
      )
      finish()
    } catch (caught) {
      if (!alive()) return
        ...
        onEnded={(saved) => {
          setAssignments((current) =>
            current.map((assignment) =>
              assignment.id === saved.id ? saved : assignment,
            ),
          )
...
function PersonDrawer({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFailure(null)
    try {
      let saved: generated.Person
      if (editing && person) {
        saved = unwrap<generated.Person>(
          await generated.updatePerson(
            person.id,
            {
              display_name_ar: nameAr.trim(),
              display_name_en: nameEn.trim() || null,
              status: status as generated.PersonPatchStatus,
            },
            requestInit(token, {
              command: true,
          ...
          await generated.registerPerson(
            {
              ...
              display_name_ar: nameAr.trim(),
              display_name_en: nameEn.trim() || undefined,
              status: status as generated.PersonCreateStatus,
            },
...
function AssignmentDrawer({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFailure(null)
    try {
      const created = unwrap<generated.Assignment>(
        await generated.createAssignment(
          {
            person_id: personId,
            position_id: positionId,
            start_at: startIso,
            end_at: toUtcIso(endAt),
...
function EndAssignmentDrawer({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setFailure(null)
    try {
      const ended = unwrap<generated.Assignment>(
        await generated.endAssignment(
          assignment.id,
          { end_at: endIso, reason: reason.trim() },
          requestInit(token, {
            command: true,
