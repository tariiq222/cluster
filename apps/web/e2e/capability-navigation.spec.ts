...
test('platform owner sees seven direct primary links and supports LTR', async ({ page }) => {
  ...
  await expect(collapsedLabels).toHaveCount(primaryLinks.length)
  for (let index = 0; index < primaryLinks.length; index += 1) await expect(collapsedLabels.nth(index)).not.toBeVisible()
