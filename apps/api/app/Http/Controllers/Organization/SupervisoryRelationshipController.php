...
final class SupervisoryRelationshipController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        return AuthorizationApi::problem(404, 'resource-not-found', 'Not Found', 'The supervisory relationship is not available.', $correlationId);
    }
    ...
    private function create(Request $request, string $principalId, string $correlationId): JsonResponse
    {
        ...
        if ($validator->fails() || array_diff(array_keys($input), ['source_unit_id', 'target_unit_id', 'relationship_type', 'start_at', 'end_at', 'capability_codes']) !== []) {
            return AuthorizationApi::problem(422, 'invalid-supervisory-relationship', 'Unprocessable Entity', 'The supervisory relationship payload is invalid.', $correlationId);
        }
        $input = $validator->validated();
        if ($input['source_unit_id'] === $input['target_unit_id']) {
            return AuthorizationApi::problem(409, 'supervisory-relationship-conflict', 'Conflict', 'A supervisory relationship cannot target the same unit.', $correlationId);
        }
        if (DB::table('organization_units')->whereIn('id', [$input['source_unit_id'], $input['target_unit_id']])->count() !== 2) {
            return AuthorizationApi::problem(404, 'organization-unit-not-found', 'Not Found', 'The organization unit is not available.', $correlationId);
        }
        $operation = 'create-supervisory-relationship';
        $requestHash = hash('sha256', json_encode($input, JSON_THROW_ON_ERROR));
        $existing = DB::table('authorization_idempotency_keys')->where([
        ...
        if ($existing !== null) {
            ...
            return is_array($payload['data'] ?? null)
                ? AuthorizationApi::resource($payload['data'], (int) $existing->response_status, $correlationId, 1)
                : AuthorizationApi::problem(500, 'idempotency-state-unavailable', 'Internal Server Error', 'The request cannot be safely replayed.', $correlationId);
        }

        try {
            return DB::transaction(function () use ($input, $principalId, $correlationId, $key, $operation, $requestHash): JsonResponse {
                ...
                return AuthorizationApi::resource($entity, 201, $correlationId, 1);
            });
        } catch (QueryException $exception) {
            return (string) $exception->getCode() === '23000'
                ? AuthorizationApi::problem(409, 'supervisory-relationship-conflict', 'Conflict', 'The supervisory relationship conflicts with existing state.', $correlationId)
                : AuthorizationApi::problem(500, 'organization-write-failed', 'Internal Server Error', 'The organization change could not be saved.', $correlationId);
        } catch (Throwable) {
            return AuthorizationApi::problem(422, 'invalid-supervisory-relationship', 'Unprocessable Entity', 'The supervisory relationship payload is invalid.', $correlationId);
        }
    }
