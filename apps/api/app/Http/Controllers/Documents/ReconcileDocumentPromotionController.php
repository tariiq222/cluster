...
final class ReconcileDocumentPromotionController
{
    ...
    public function __invoke(Request $request, string $versionId): JsonResponse
    {
        ...
        try {
            ...
            $actor = DocumentsApi::actorOrProblem($principal, $this->access, $facts, DocumentUploadHandler::RECONCILE_PROMOTION_OPERATION, $correlationId);
            if ($actor instanceof JsonResponse) {
                return $actor;
            }
            $result = $this->handler->reconcilePromotion(
                $actor,
                $versionId,
                new IdempotencyContext($actor->principalId, DocumentUploadHandler::RECONCILE_PROMOTION_OPERATION, $idempotencyKey, hash('sha256', json_encode($semantics, JSON_THROW_ON_ERROR))),
            );
