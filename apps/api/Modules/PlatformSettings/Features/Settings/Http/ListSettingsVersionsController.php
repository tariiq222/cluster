...
final class ListSettingsVersionsController
{
    ...
    {
        $context = $this->api->authorize($request, 'platform_settings.read', $this->api->facts('platform_settings_version'));
        if ($context instanceof JsonResponse) {
            return $context;
        }
        $items = $this->settings->listVersions();
        foreach ($items as &$item) {
            $item['allowed_actions'] = $context['decision']->allowedActions;
        }
