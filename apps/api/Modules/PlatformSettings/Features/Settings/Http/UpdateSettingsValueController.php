...
final class UpdateSettingsValueController
{
    public function __construct(private readonly PlatformSettingsApi $api, private readonly PlatformSettingsHandler $settings) {}

    public function __invoke(Request $request, string $versionId, string $settingKey): JsonResponse
    {
        ...
        if (! $request->has('value')) {
            return $this->api->problem(422, 'validation-failed', 'Unprocessable Content', 'A typed value is required.', $context['correlation_id']);
        }
        try {
            $body = $this->settings->setValue($versionId, $settingKey, $request->input('value'), $etag);
