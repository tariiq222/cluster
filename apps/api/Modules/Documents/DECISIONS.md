...
- `composer --working-dir=apps/api test` against the working tree containing this slice → 788 tests, 777 passed, 9 skipped, **2 failed**. Both failures are pre-existing on the un-modified baseline (verified by `git stash -u` + re-run):
  ...
  Root cause is drift between `Makefile:102-119` and the test surface: the test (CiMakeSurfaceTest.php:61-94) asserts the strings `'docs/validate-docs.sh is missing'` and `'docs/catalog.yaml is missing'`, but the Makefile currently emits `'scripts/validate-docs.sh is missing.'` and deliberately dropped the catalog probe (Makefile:102 comment: "The current lean docs tree has no catalog or MkDocs registry"). The 114 unfound references in deferred plans are unrelated — this slice's validator carve-out already addresses that. Closing the two CiMakeSurfaceTest s9 failures is its own slice: update the test assertions to match the current Makefile recipe, OR reintroduce the catalog probe if a governed docs catalog is in scope. Neither path is in this slice.
