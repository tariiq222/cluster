...
namespace Modules\Documents\Http;
...
final class DocumentsApi
{
    ...
    {
        return match (true) {
            $exception instanceof \Modules\Documents\Features\Upload\StaleDocumentLockVersion => self::problem(412, 'precondition-failed', 'Precondition Failed', 'The document changed after it was read.', $correlationId),
            in_array($exception->getMessage(), ['document_access_denied', 'linked_resource_access_denied', 'linked_resource_facts_unavailable'], true) => self::problem(403, 'access-denied', 'Forbidden', 'Access denied.', $correlationId),
            in_array($exception->getMessage(), ['upload_intent_not_found', 'document_version_not_found', 'document_owner_organization_mismatch'], true) => self::problem(404, 'document-upload-not-found', 'Not Found', 'The document upload is not available.', $correlationId),
