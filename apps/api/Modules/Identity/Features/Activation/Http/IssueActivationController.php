...
namespace Modules\Identity\Features\Activation\Http;
...
use Modules\Identity\Features\Authentication\Http\IdentityIdempotency;
...
final class IssueActivationController
{
    ...
    public function __invoke(Request $request, string $accountId): JsonResponse
    {
        ...
        if (($session['restricted'] ?? false) === true || ! $this->authorization->canIssueActivation($principal)) {
            return IdentityApi::problem(
                403,
                'access-denied',
                'Forbidden',
