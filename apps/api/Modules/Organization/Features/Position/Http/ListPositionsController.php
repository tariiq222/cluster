...
final class ListPositionsController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $validator = Validator::make($query, [
            ...
            'unit_id' => ['sometimes', 'string', 'regex:/\A[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\z/'],
        ]);
        if ($validator->fails() || array_diff(array_keys($query), ['cursor', 'limit', 'unit_id']) !== []) {
        ...
        try {
            $page = $this->handler->list(
                ...
                function (string $positionId) use ($principal): bool {
                    $facts = $this->resourceFacts->factsForPosition($positionId);

                    return $facts !== null
                        && $this->access->decide($principal, 'organization.position.read', $facts)->isAllowed();
                },
            );
        ...
        if ($page['next_cursor'] !== null) {
            $response->header('Link', '</api/v1/organization/positions?'.http_build_query(array_filter([
