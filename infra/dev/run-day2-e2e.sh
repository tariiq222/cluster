...
readonly TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/cluster-day2-e2e.XXXXXX")"
readonly DATABASE="$TEMP_DIR/database.sqlite"
readonly API_PORT="${DAY2_API_PORT:-$(pick_free_port)}"
readonly WEB_PORT="${DAY2_WEB_PORT:-$(pick_free_port)}"
...
env W1_1_API_ORIGIN="http://127.0.0.1:${API_PORT}" W1_1_WEB_PORT="$WEB_PORT" \
  npm run test:e2e:local -- e2e/day2-workflow.spec.ts
