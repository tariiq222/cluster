...
description: Draft unified design system for an Arabic-first internal operations platform.
colors:
  canvas: "#F2FAFC"
  surface: "#FFFFFF"
  ink: "#0B2F3A"
...
>
> هذه مسودة غير معتمدة مبنية على تحليل `rrrr.html` وقرارات المالك. لا يبدأ تطبيقها، ولا تضاف الحزم أو الأصول، ولا ينشأ ملف `.impeccable/design.json` قبل الموافقة الصريحة.
...
- لا يوجد accent زخرفي ثان. تستخدم طبقات `canvas` و`surface` و`border` لبناء التسلسل الهرمي.
...
- **Floating Control** (`0 6px 22px rgb(11 47 58 / 12%)`): قائمة أو popover أو عنصر عائم فقط.
...
- **Corner Style:** `12px` موحد للألواح.
