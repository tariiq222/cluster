...
| A2 | فريق التطوير من 2 إلى 4 أشخاص بخبرة جيدة في Laravel وأقل نضجاً في React وKubernetes | مراجعة الفصل بين المعمارية والمنتج مع كل تعيين | يعاد تقدير خطة الإصدار |
| A3 | المستخدم النهائي يدخل من داخل الشبكة الداخلية على متصفح حديث | فحص الجلسة من سجلات الإنتاج | يُمنع الدخول الخارجي ويُحظر إتاحة الواجهة |
...
| الرقم | الافتراض | معيار التحقق | الأثر عند الانتهاك |
|---|---|---|---|
| T1 | المنصة تعمل بالكامل داخل مركز بيانات التجمع دون اتصال إنترنت خارجي | فحص الشبكة وإعدادات `NetworkPolicy` | يُمنع النشر ويُعدل التصميم |
| T2 | MySQL HA داخل Kubernetes قادر على خدمة 2,000 مستخدم متزامن و20,000 حساب | اختبار تحميل على بيئة قريبة من الإنتاج | يُرفع عدد النسخ أو يُعدل المعمار |
| T3 | صور الحاويات تُسحب من Registry داخلي ولا تُحمَّل وقت التشغيل | فحص `NetworkPolicy` وسجل البناء | يُمنع الإصدار |
| T4 | Composer وnpm يستخدمان مرايا داخلية فقط | فحص سكربتات البناء | يُمنع الإصدار |
...
| T8 | التخزين الداخلي متوافق مع S3 ومحلي داخل المركز | اختبار استعادة من لقطة محلية | يُمنع الإصدار |
| T9 | محرك البحث الداخلي لا يحتاج اتصالاً خارجياً ولا ترخيصاً سحابياً دورياً | مراجعة فنية لمنتج البحث قبل اختياره | يُستبدل المنتج |

## 4. الافتراضات التشغيلية

| الرقم | الافتراض | معيار التحقق | الأثر عند الانتهاك |
|---|---|---|---|
| O1 | فريق عمليات متفرغ للنظام داخل مركز البيانات | توافر الفريق في خطة الطوارئ | تُخفف أهداف RPO وRTO |
| O2 | النسخ الاحتياطي في مخزن مستقل عن Kubernetes ومشفر | فحص المخزن وموقعه | يُمنع تشغيل بيانات حقيقية |
| O3 | بيئة اختبار وبيئة تجريبية منفصلتان قبل الإنتاج | فصل شبكي وملكي واضح | يُمنع النشر في الإنتاج |
| O4 | صيانة Kubernetes تتم في نافذة معلومة دون التأثير على الخدمة | خطة صيانة معتمدة | تُعدَّل النافذة |
| O5 | تحديث الحزم يتم بمسار Offline موثق | سجل تحديثات الحزم | يُمنع التحديث دون إجراء موثق |

## 5. القيود الصارمة Hard Constraints

| الرقم | القيد | الاستثناء المسموح | المرجع |
|---|---|---|---|
| C1 | لا اتصال بالإنترنت الخارجي في أي بيئة تشغيلية | اتصالات إدارية محددة بإجراء موثق ومراقب | ADR-018 |
| C2 | لا CDN ولا خطوط خارجية ولا سكربتات عامة في الواجهة | لا استثناء | ADR-018 |
| C3 | لا SaaS خارجي يعتمد على ترخيص سحابي دوري للتحقق | لا استثناء | ADR-018 |
...
| OOS11 | بريد إلكتروني وSMS وWhatsApp للإشعارات | تعقيد تشغيلي وأمني في الوضع المعزول | عند توفر بوابة معتمدة |
...
| R4 | تعطل Kubernetes يؤثر على الخدمة | منخفض | عالٍ | عدة نسخ وMySQL HA واختبار استعادة | مسؤول العمليات |
...
| R8 | تأخر ترقية حزم بسبب المسار المعزول | عالٍ | متوسط | جدول ترقية ربع سنوي بمسار Offline موثق | مسؤول العمليات |
