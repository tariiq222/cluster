...
| TB-11 | قناة CI/CD والبناء المعزول | بيئة التطوير/البناء والتسجيل الداخلي | بناء بدون إنترنت، SBOM، توقيع صور |
| TB-12 | قناة الخروج Egress | الكتلة والإنترنت الخارجي | ممنوعة افتراضياً عبر NetworkPolicy |
...
### 4.10 TB-11 قناة CI/CD المعزولة

| الفئة | التهديد | الضابط | الاختبار |
|---|---|---|---|
| S | تزوير صورة حاوية | توقيع صور OCI، تحقق عند الـdeploy | `SupplyChainTest::image_signature_verified` |
| T | تعديل كود بعد التوقيع | SBOM مولّد لكل إصدار، فحص ثغرات قبل النشر | `SupplyChainTest::vulnerability_scan_blocks_publish` |
| R | إنكار نشر | سجل نشر بنسخة الإصدار وSHA | `DeployTest::deploy_logged_with_artifacts_hash` |
| I | تسرب أسرار في الصورة | فحص أسرار في الـpipeline، منع متغيرات بيئة تحتوي أسراراً | `SupplyChainTest::secrets_not_in_image` |
| D | توقف النشر بسبب تعطل المرايا | مرايا composer/npm داخل المركز، حزم مرفوعة مسبقاً | `AirGapTest::build_uses_only_internal_mirrors` |
| E | تنفيذ تعليمات في طبقة البناء | تشغيل البناء داخل شبكة معزولة بدون إنترنت | `AirGapTest::build_network_isolated` |

### 4.11 TB-12 قناة الخروج Egress

| الفئة | التهديد | الضابط | الاختبار |
|---|---|---|---|
| S | خادم خبيث يتصل بالإنترنت | NetworkPolicy ترفض egress افتراضياً، allowlist فقط | `NetPolTest::default_deny_egress` |
| T | تحديث من مصدر خارجي | لا تحديث أثناء التشغيل، فقط من مرايا داخلية موقعة | `AirGapTest::runtime_pulls_blocked` |
| R | إنكار اتصال | كل اتصال مسجل في تدفق الشبكة | `NetPolTest::egress_attempts_logged` |
| I | تسرب بيانات عبر DNS | DNS داخلي فقط، تحليل وحجب النطاقات الخارجية | `NetPolTest::dns_external_blocked` |
| D | قطع الخدمة بسبب حجب مفرط | اختبار دوري لقواعد السماح قبل التغيير | `NetPolTest::allowlist_validated` |
| E | مهاجم يستخدم egress للتصعيد | التحقق من أن مسارات egress لا تلامس بيانات حساسة | `NetPolTest::egress_targets_are_non_data_systems` |
...
| نقل البيانات | عدم نقل خارج المملكة | لا egress، كل البيانات داخل المركز | `AirGapTest::no_external_data_transfer` |
...
| 1-6 | إدارة الثغرات | فحص دوري، SBOM، تحديث من مرايا داخلية | `SupplyChainTest::*` |
| 1-7 | التسجيل والمراقبة | سجلات مركزية، تنبيهات، حفظ 12 شهراً | `LoggingTest::*` |
| 1-8 | حماية البنية التحتية | NetworkPolicy، فصل VLAN، air-gap | `AirGapTest::*` |
