...
## final — Task 14 route splitting

| | الحجم (minified · gzip) |
|---|---|
| قبل التقسيم — الحزمة الرئيسية (المقاس المُسلَّم في الخطة) | 1,493.93 kB · 416.05 kB |
| بعد التقسيم — الحزمة الرئيسية (`index-*.js`) | 577.55 kB · 173.22 kB |
| أكبر حزمة مسار (`ReportsMonitoringScreen-*.js`) | 398.89 kB · 115.67 kB |

حُوِّلت شاشات المسارات الأربع عشرة إلى `React.lazy` + `Suspense` بحاجز مشترك واحد (هيكل `LoadingState` المشترك مع `role="status"` و`aria-live="polite"` ونص `shellCopy[locale].loading` محلي). انخفضت الحزمة الرئيسية من 1,493.93 kB إلى 577.55 kB minified (−916.38 kB) ومن 416.05 kB إلى 173.22 kB gzip (−242.83 kB).

تحذير «أكبر من 500 kB» **لم يختفِ**: الحزمة الرئيسية (577.55 kB) ما زالت فوق العتبة وهي الآن الحزمة الوحيدة فوقها؛ شاشات المسارات جميعها في حزم مستقلة (أكبرها `ReportsMonitoringScreen` مع مكتبة المخططات: 398.89 kB). الحجم المتبقي موجَّه بالخطة: الغلاف `AppShell` ومكوّنات الصدفة/الحالة ومكتبات النواة (react/react-dom/react-router/react-query/sonner/radix) تبقى eager حسب خطوة ١ من المهمة ١٤، ولم تُرفع قيمة العتبة في Vite.

ملاحظة إعادة الإنتاج: `npm run build` يتوقف حالياً عند بوابة `tsc -b` بسبب ٥ أخطاء سابقة الوجود في `AssignmentSheet.tsx` (عمل قيد التطوير غير مرتبط بهذه الخطوة)؛ الأرقام أعلاه مقيسة عبر `vite build` مباشرةً.

### Step 3 — Confirm no legacy remains — 2026-08-02

| الفحص | النتيجة |
|---|---|
| استيرادات `from '…/ui'` بصيغة الاقتباس المفرد أو المزدوج تحت `apps/web/src` | 0 |
| محتويات `apps/web/src/styles/` | `index.css` و`theme.css` فقط |
| `apps/web/src/ui/index.tsx` | غير موجود (مجلد `src/ui` كاملاً غائب) |

كل فحوصات الخطوة ٣ مطابقة للتوقعات. بوابة الخطوة ٢ لم تُسجَّل في `baseline.md` لأنها لم تخرج 0 (فشل `test:unit` و`coverage` في اختبار واحد داخل `AccessScreen.test.tsx`، وفشل جولة e2e المحلية 29/23/1 ضد API قائمة منذ زمن خارج حامل W1.1) — النتائج الحرفية في تقرير التنفيذ TASK14-FINAL-GATES.
