...
| | |
|---|---|
| **المسار** | `/documents · /documents/new · /documents/:documentId` |
| **النمط** | قائمة ← تفصيل |
...
**القائمة**: `DataTable` بأعمدة العنوان، النوع، الحالة، آخر تعديل. شريط أدوات فيه بحث وفلتر حالة. زر «أنشئ مستنداً» ينقل إلى `/documents/new` — صفحة كاملة مستقلة، لا `Sheet` ولا `Dialog`.
**الإنشاء (`/documents/new`)**: صفحة كاملة، نموذج واحد يمتدّ على عرض `PageLayout` (لا جزيرة `max-w-2xl` ضيّقة). **التخطيط**: على المكتب شبكة منطقيّة من منطقتين متجاورتَين — سطح الإدخال الرئيسي ~٢/٣ على اليمين اللوغاريتمي الأول وسطح المراجعة والإجراءات ~١/٣ كـ`card` مسطّحة شقيقة (لا متداخلة)، وتستقرّ بطاقة المراجعة `lg:sticky` كي يبقى الإجراء الأساسي في النطاق المرئي عند ١٤٤٠×٩٠٠؛ على الجوال ينهار الترتيب إلى عمود واحد بترتيب الـDOM. سطح الإدخال بطاقة مسطّحة واحدة (`rounded-xl border bg-card`) تتقاسمها ثلاثة أقسام يفصل بينها `border-t pt-6`: (1) **البيانات الأساسية** — العنوان يقترحه النظام تلقائياً من اسم الملف (قابل للتعديل، حتى 255 حرفاً)، الوصف الاختياري (حتى 2000 حرف)، التصنيف (افتراضياً `internal`). (2) **المنشأة المالكة** — اختيار منشأة من `principal.availableScopes` المفلترة على `scopeType === 'facility'` ومُدمَج فيها المنشأة الفعّالة الحالية؛ التسمية البشرية فقط، لا `UUID` ولا مفتاح سياسة. قائمة المنشأة والـ`Select` الخاص بالتصنيف كلاهما `w-full` ليملأ العمود المُسنَد. الاختيار يستدعي `principal.selectScope('facility', id)` ويظهر كحالة «جارٍ تبديل المنشأة» حتى تصبح المنشأة الجديدة هي الفعّالة على الخادم (توجيه عبر `scopeReady`). (3) **الملف الأول** — إلزامي، ≤ 1 غيغابايت، مع رسائل موضّحة عند الفراغ أو تجاوز الحجم؛ إذا تُرك العنوان فارغاً تستخدم التهيئة العامة اسم الملف. **مُنتقي الملف موضْعَن بالكامل** — `Button` مرئي يستخدم `FileUp` ونص `اختيار ملف`/`استبدال الملف` حسب الحالة، ويستحثّ `<input type="file">` أصلي يبقى في الـDOM (مخفي بصرياً) للاختبارات والتقنية المساعدة؛ لا تظهر أداة المتصفح الأصلية الإنجليزية. (4) **مراجعة قبل الإنشاء** — بطاقة `<dl>/<dt>/<dd>` دلالية في سطح المراجعة تعرض العنوان، التصنيف، المنشأة، اسم/حجم الملف، وتذكير بأن سياسة الحماية والاحتفاظ تُطبَّق تلقائياً من التصنيف. أسماء الملفات الطويلة تُغلَّف داخل `<bdi dir="auto">` لمنع فيضان العمود الضيّق. تقدّمٌ واحد يدمج المراحل (تجزئة ← تهيئة ← رفع ← فحص ← إتمام) ويُعطّل الحقول أثناء التشغيل لمنع التكرار؛ منطقة `role="status" aria-live="polite"` تُعلن المرحلة. الإجراء الأساسي «أنشئ المستند» يملأ عرض عمود المراجعة وفوقه فاصل `Separator`، وثانوي «إلغاء» يعود إلى `/documents`. تُحترم قاعدة «منشأة تابعة»؛ عند انعدام أي منشأة متاحة تُعرض `Alert` غير مُفصِحة مع زر «عودة إلى المستندات»، ويُخفى زر الإرسال. تُحتفظ بحالة الرفع بعد فشل ما بعد التهيئة، فتعيد المحاولة في الجلسة استخدام `upload_id` و`upload_url` و`required_headers` ذاتها دون إنشاء مستند جديد.
**التفصيل**: صفحة كاملة، رأس فيه العنوان والحالة وأزرار الانتقال، ثم تبويبات: المعاينة · النسخ · الروابط · الوصول.
**الرفع** عملية من ثلاث خطوات (تهيئة ← رفع ← إتمام) تُعرض كشريط تقدّم واحد في **صفحة كاملة** `/documents/:documentId/versions/new`، لا كثلاث شاشات — المستخدم يراها فعلاً واحداً. حالة الفحص من الفيروسات تظهر كـ`Badge` مع أيقونة، لا كلون مخصّص. **الرفع الجديد يستخدم نقطة النهاية `/documents/{id}/versions`** المخصصة للإصدارات، لا التهيئة العامة، ويُحترم `intent.method` و`intent.required_headers` حرفياً دون استبدال.
...
**شجرة `Collapsible` لا جدول** — العلاقة الأبوية هي المعلومة الأساسية وتضيع في جدول مسطّح. كل عقدة: الاسم، الرمز، `Badge` الحالة، وقائمة `DropdownMenu` للإجراءات. **الإنشاء في صفحة كاملة** `/organization/units/new` مع `parentId` اختياري في الرابط يحدّد الأب مسبقاً، والتحرير في صفحة `/organization/units/:unitId/edit`.
...
`DataTable`: الاسم، الرمز، النوع، الحالة. **الإنشاء في صفحة كاملة** `/organization/facilities/new` **والتحرير في** `/organization/facilities/:facilityId/edit`. تعارض التحديث المتزامن (409) يظهر كـ`Alert` في الصفحة مع إبقاء المُدخَل — لا تُفرَّغ الصفحة.
...
`DataTable`: المسمّى، الوحدة، الحالة، المشغول/الشاغر. فلتر بالوحدة. **الإنشاء في صفحة كاملة** `/organization/positions/new` مع `unitId` اختياري يحدّد الوحدة مسبقاً، **والتحرير في** `/organization/positions/:positionId/edit`.
...
جدول بسيط: الاسم، الرمز، الحالة. مرجع صغير — لا يحتاج فلاتر ولا بحث متقدّم. **الإنشاء في صفحة كاملة** `/organization/job-titles/new`.
...
`DataTable`: الاسم، الرقم الوظيفي، الحالة، الوحدة. بحث بالاسم والرقم. **التسجيل في صفحة كاملة** `/organization/people/new` **والتفصيل والتحرير في صفحة** `/organization/people/:personId` بتبويبين: البيانات · التكليفات.
...
`DataTable`: اسم المستخدم، الشخص المرتبط، الحالة، آخر دخول. **الإنشاء في صفحة كاملة** `/access/accounts/new` — صفحة تعرض اختيار الموظف واسم الدخول ونتيجة الإنشاء. **الإدارة في صفحة كاملة** `/access/accounts/:accountId` تعرض الحالة وتُظهر إجراءات الحالة المتاحة من `allowed_actions`.
...
تفصيل الحدث في **صفحة كاملة** `/reports/audit/events/:eventId` يعرض الحمولة كاملة مع التنقية وإخفاء الأسرار، والعودة تعيد المدقّق إلى نفس الموضع في السجل.
...
قائمة المفاتيح مع قيمها، **وتحرير القيمة في صفحة كاملة** `/platform/settings/:versionId/security/:settingKey/edit` بحقل يتبدّل حسب `value_type` — مع شريط علوي يبقي الإصدار والحالة مرئيين.
...
قائمة التقاويم بالنطاق، والمحدَّد يُعرض بجزأين: **أيام الأسبوع** (سبعة صفوف: يوم عمل؟ + وقت البداية/النهاية) و**الاستثناءات** (تقويم شهري بصري تُعلَّم فيه أيام الاستثناء). **الإنشاء في صفحة كاملة** `/platform/calendars/new`، **وضبط يوم الأسبوع في صفحة** `/platform/calendars/:calendarId/weekdays/:weekday/edit`، **والاستثناء في صفحة** `/platform/calendars/:calendarId/exceptions/new`. النشر بتأكيد.
...
قائمة النوافذ: من، إلى، السبب، الحالة. **الجدولة في صفحة كاملة** `/platform/maintenance/new` مع الإبقاء على قائمة النوافذ الجارية في الصفحة الأصلية. النافذة الجارية تُعلَّم بأيقونة وتُثبَّت أعلى القائمة.
...
قائمة السياسات مع مفتاح `Switch` للتفعيل **وتحرير السياسة في صفحة كاملة** `/platform/alerts/:policyId/edit`.
