...
- تعمل على خادم داخلي يدار عبر Dokploy، ويقتصر الوصول إليه على منافذ ومسارات معتمدة.
...
- نشر داخلي محدود المنافذ عبر Dokploy؛ لا يدّعي Air-gap أو HA عند فشل الخادم.
...
13. **تشغيل ذاتي مقيد:** Dokploy يدير حزمة Docker Compose مثبتة، ولا تنشر خدمات الحالة أو الإدارة للعامة، ولا تعتمد الواجهة على CDN أو سكربتات عامة وقت التشغيل.
...
- Dokploy وTraefik على خادم داخلي واحد أمام React Unified Web App وLaravel Web/API.
- Queue Workers وScheduler وحيد، كلها معرفة في Docker Compose مثبت الإصدارات.
- MySQL وValkey وخدمات الحالة على شبكات Docker داخلية بلا منافذ عامة.
- Logs وMetrics وتنبيهات تناسب الخادم الواحد ولا تفترض عنقوداً خارجياً.
- الصور مثبتة بالـdigest، والبناء يعتمد lockfiles ومسار تحديث مراجع.
...
تُحسم قبل بوابة التشغيل المعنية: موقع مخزن النسخ الخارجي، سعة الخادم وضبط MySQL وOpenSearch والسجلات، مدد الاحتفاظ التفصيلية، والتصنيف الأمني الرسمي. منصة النشر محسومة في ADR-023 إلى Dokploy وDocker Compose؛ ولا تغير هذه القرارات ملكية البيانات أو DAG أو عقود الموديولات.
