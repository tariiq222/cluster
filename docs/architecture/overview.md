...
- تعمل داخل مركز بيانات التجمع دون اعتماد على الإنترنت الخارجي.
...
- الخبرة الأقوى في Laravel، مع نضج أقل في React وKubernetes.
- من 5,000 إلى 20,000 حساب وحتى 2,000 مستخدم متزامن.
- MySQL عالي التوافر داخل Kubernetes.
- نشر وتشغيل معزولان عن الإنترنت.
...
13. **تشغيل Air-gapped:** لا CDN ولا خطوط ولا سكربتات ولا تنزيل حزم من الإنترنت أثناء البناء أو التشغيل داخل البيئة المعزولة.
...
- React Unified Web App خلف موازن داخلي.
- Laravel Modular Monolith بعدة Web/API replicas.
- Queue Workers بعدة replicas وScheduler وحيد أو leader-elected.
- MySQL HA، وValkey للـCache/Queue، وS3-compatible Object Storage، وOpenSearch للبحث الموجّه للمستخدم.
- Loki للسجلات التشغيلية، وMetrics وAlerts مركزية.
- Registry ومرايا Composer وnpm داخلية.
- مخزن نسخ احتياطي داخل المركز ومستقل عن نطاق عطل Kubernetes.
...
- اختبارات حمل حتى 2,000 مستخدم متزامن وفشل عقدة واستعادة البيانات والملفات.

## 14. قرارات مؤجلة لا تغير الحدود

تُحسم قبل بوابة التشغيل المعنية: منتج التخزين، توزيع Kubernetes، موقع مخزن النسخ، سعة وضبط OpenSearch وLoki، مدد الاحتفاظ التفصيلية، والتصنيف الأمني الرسمي. يجب أن تظل البدائل متوافقة مع التشغيل المعزول، وملكية البيانات، وDAG، وعقود هذه الحزمة.
