...
- **W1.3 قيد التنفيذ**: خط `work-1-3` (bookmark عند `0a4c352c`) يحتوي capability
  catalog وRBAC persistence والعلاقات الإشرافية وسياسات الحقول وتدقيق الوصول
  الحساس، وبعد دمج خط `cluster-w13` أيضاً واجهة React الإدارية للـAuthorization
  وطبقة HTTP (قرار الوصول وتفسيره وإدارة التعيينات والعلاقات). غير مدمج في
  `main` بعد؛ worktree ‏`cluster-w13` ما زال قائماً وفيه جلسة تطوير نشطة، ويزال
  بعد اكتمالها ودمج ما تبقى.
